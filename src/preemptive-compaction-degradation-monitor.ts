...
import type { DegradationMonitorConfig, PreemptiveCompactionConfig } from "./config"
...
export function createPostCompactionDegradationMonitor(args: {
  ...
  const { client, directory, config, tokenCache, compactionInProgress } = args
  const monitor: DegradationMonitorConfig = config.degradationMonitor
  ...
  const onSessionCompacted = (sessionID: string): void => {
    ...
    postCompactionRemaining.set(sessionID, monitor.monitorCount)
  ...
  const triggerRecovery = async (sessionID: string): Promise<void> => {
    ...
    if (recoveryCount >= monitor.maxRecoveryAttempts) {
    ...
    suppressRecoveryCompactionUntil.set(sessionID, Date.now() + monitor.recoverySuppressionMs)
    ...
    try {
      ...
      await withTimeout(
        ...
        monitor.timeoutMs,
        `Compaction recovery summarize timed out after ${monitor.timeoutMs}ms`,
      )
  ...
  const onAssistantMessageUpdated = async (info: AssistantCompactionMessageInfo): Promise<void> => {
    ...
    if (nextStreak >= monitor.noTextThreshold) {
